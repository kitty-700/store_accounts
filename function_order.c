...
void Add(DB * root)
{
	...
	select_in_first_page = Select_SA_Number(root,
		...
		/*0번 선택시*/	"새로운 사이트 등록.",
		SHOW_LIST);
	MAKE_EXTENDED_BUFFER(history_record_1);
	...
	switch (select_in_first_page)
	{
		...
		case MAKE_NEW_SITE:
			...
			/*┌함수 성공┐*///-> 사이트 추가 성공
			sprintf_s(history_record_1, COMMON_LENGTH_EXTENDED, "[%03d] %s 사이트 추가", root->how_many_sites + 1, new_SA->Site_Name);
			sprintf_s(history_record_2, COMMON_LENGTH_EXTENDED, "");
	...
	sprintf_s(history_record_2, COMMON_LENGTH_EXTENDED, "\t[%03d]\tID : %s\n\t\t\tPW : %s", FIRST, new_AE->ID, new_AE->PW);
...
void Update(DB * root)
{
	...
	switch (select_in_second_page)
	{
		...
		case CHANGE_SITE_NAME:
			...
			sprintf_s(history_record_1, COMMON_LENGTH_EXTENDED, "[%03d]\t\" %s \" 사이트의 이름을", select_in_first_page, to_be_updated_SA->Site_Name);
			
			SET_CONSOLE_COLOR(ORDER_COLOR);
			Get_String_Without_Buffer_Overflow(string_to_be_replaced);
			SET_CONSOLE_COLOR_DEFAULT;
			...
			sprintf_s(history_record_2, COMMON_LENGTH_EXTENDED, "\t\" %s \" 로 업데이트했습니다.", to_be_updated_SA->Site_Name);
	...
	case 3: //메모 변경
		...
		break;
	FO_ERROR_1:
	default: //에러 (Select_AE_Attribute()에서 OUT_OF_RANGE 예외로 걸려졌으므로 나올 수 없음.)
	...
	{
	case 1: //ID 변경
		sprintf_s(history_record_1, COMMON_LENGTH_EXTENDED, "[%03d] %s 사이트의\n\t\t[%03d] ID : %s 의 ID 를\n\t\t\t\" %s \" 에서",
	...
	case 3: //메모 변경
		...
		break;
	default:
		BEFORE_RETURN_PRINT_("에러!!\n");	//앞선 FO_ERROR_1: 레이블의 이유와 같다.
	}
	sprintf_s(history_record_2, COMMON_LENGTH_EXTENDED, "\t\t\" %s \" 로 변경.", string_to_be_replaced);
	/*┌함수 성공┐*///-> 계정 정보 업데이트 성공
	...
	switch (select_in_second_page)
	{
		...
		case DELETE_SITE:
		{
			if (Ask_Continue("삭제하시겠습니까 ?") == FALSE)
				BEFORE_RETURN_PRINT_(MESSAGE_CANCEL(삭제));
			sprintf_s(history_record_1, COMMON_LENGTH_EXTENDED, "[%03d] %s 사이트를 삭제했습니다.", select_in_first_page, delete_SA->Site_Name);
			sprintf_s(history_record_2, COMMON_LENGTH_EXTENDED, "");
		...
		{
			sprintf_s(history_record_1, COMMON_LENGTH_EXTENDED, "[%03d] %s 사이트의", select_in_first_page, delete_SA->Site_Name);
			...
			if (Ask_Continue("삭제하시겠습니까?") == FALSE)
				BEFORE_RETURN_PRINT_(MESSAGE_CANCEL(삭제));
			sprintf_s(history_record_2, COMMON_LENGTH_EXTENDED, "\t[%03d] ID : %s 계정을 삭제했습니다.", select_in_second_page, delete_AE->ID);
...
void Swap_Site_Order(DB * root)
{
	...
	SET_CONSOLE_COLOR_DEFAULT;
	printf("의 위치를 교환했습니다.\n");
	sprintf_s(history_record_1, COMMON_LENGTH_EXTENDED, "[%03d] %s 와(/과)", select_1, SA_1->Site_Name);
	sprintf_s(history_record_2, COMMON_LENGTH_EXTENDED, "[%03d] %s 의 위치를 교환했습니다.", select_2, SA_2->Site_Name);
...
{	//1~2개의 문자열을 CS 구조체에 단순히 붙이는 정도
	...
	if (STRING_INPUT(str_2))
		...
		strcpy_s(temp_CS->change_to, COMMON_LENGTH_EXTENDED, "");
	////////////////////////////////////////////////////////////
	if (HISTORY_EMPTY)
...
int History_Show()
{
	...
	SET_CONSOLE_COLOR(HISTORY_COLOR);
	while (temp_CS != NULL)
	{
